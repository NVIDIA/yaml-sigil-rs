...
BUF_ADAPTER_PATH = MODULE_PATH.parent.parent / "trusted-cargo" / "buf-tools"
...
class WorkflowStructureTests(unittest.TestCase):
    ...
    def test_terminal_setup_uses_elevated_and_native_paths(self) -> None:
        ...
        self.assertIn('paths = ["/policy/.github/trusted-cargo/buf-tools"]', shell)
    ...
    def test_protected_buf_adapter_only_returns_the_authenticated_path(self) -> None:
        manifest = (BUF_ADAPTER_PATH / "Cargo.toml").read_text(encoding="utf-8")
        library = (BUF_ADAPTER_PATH / "src" / "lib.rs").read_text(encoding="utf-8")
        build = (BUF_ADAPTER_PATH / "build.rs").read_text(encoding="utf-8")

        self.assertIn('name = "buf-tools"', manifest)
        self.assertIn('version = "1.72.0-hotfix.1"', manifest)
        self.assertIn("publish = false", manifest)
        self.assertIn('PathBuf::from("/trusted-tools/bin/buf")', library)
        self.assertNotIn("std::process", library)
        self.assertNotIn("std::env", library)
        self.assertEqual(build.splitlines()[-1], "fn main() {}")

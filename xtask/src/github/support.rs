...
pub(super) fn source_version(root: &Path) -> Result<Version, String> {
    crate::versions::current(root).map_err(|error| error.to_string())
}

pub(super) fn validate_package_family(root: &Path) -> Result<(), String> {
    crate::release::validate_policy(root).map_err(|error| error.to_string())
}

fn verify_packages(source: &str, version: &Version) -> Result<(), String> {
    if super::registry_states(source, version)?
        .iter()
        .any(|published| !published)
    {
        return Err(
            "support activation requires every exact source archive to be published".into(),
        );
    }
    Ok(())
}
...
mod tests {
    ...
    impl Transport for ReadOnlyGithub {
        ...
        fn post<T: serde::de::DeserializeOwned, P: Serialize>(
            &mut self,
            _path: &str,
            _payload: &P,
        ) -> Result<T, String> {

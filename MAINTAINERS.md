...
Use this runbook for maintainer operations in `yaml-sigil-rs`. Read
...
For canonical release pull requests, CI removes the untracked root
`Cargo.lock` generated by dependency inspection before validating release
source. It requires a regular file, rejects symlinks and tracked lockfiles,
and lets the normal CI suite resolve its ignored lockfile afterward.
...
The synthetic `dev/0.6.0` coordination line is obsolete. Do not synchronize,
promote, or accept further contributions to it. Future `0.6.x` work targets
`main` unless `support/0.6` is separately activated.
...
cargo xtask github release start-support \
  --repository NVIDIA/yaml-sigil-rs --version MAJOR.MINOR.PATCH
```
...
Recovery requires at least one published source package. Start a fresh
release from the current tip when nothing was published. To resume partial
publication, require a dependency-ordered prefix with no tags or Releases.
Verify that every selected crate is public before creating a tag or Release.
Investigate a Release without its exact retained tag, a conflicting tag, or a
mismatched source archive; do not replace those objects to force recovery.

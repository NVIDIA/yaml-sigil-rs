...
`yaml-sigil-rs`. Contributor and release procedures live in
...
2. Prepare one signed, DCO-compliant activation commit from exact `main`.
   The bounded xtask selects the non-release `${target}-rc.0` safety stub,
   synchronizes internal workspace requirements, and accepts only the root
   manifest change. Release-plz first participates after promotion when a
   maintainer prepares an actual release:
...
6. Require the automatic coordination-ref CI at the exact activation SHA to
   succeed, including advisory hosts, and require Publish to no-op. Require

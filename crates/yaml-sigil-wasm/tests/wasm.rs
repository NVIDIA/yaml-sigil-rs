...
use yaml_sigil_wasm::{
    ArtifactResourceLimits, compose, compose_with_limits, decompose, decompose_with_limits, sign,
    sign_with_limits, verify, verify_with_limits,
};
...
fn finite_limit(maximum: usize) -> ArtifactResourceLimits {
    ArtifactResourceLimits::new()
        .with_max_artifact_bytes(wasm_bindgen::JsValue::from_f64(maximum as f64))
...
fn bounded_round_trips_use_exact_input_and_output_limits() {
    ...
    for (algorithm, (private, public)) in [(ED25519, ed25519_keys(7)), (P256, p256_keys(3))] {
        for (form, outer) in [
            ("yaml", None),
            ("protobuf", Some("signature_strict".to_owned())),
        ] {
            let original = sign(
                bytes(PAYLOAD),
                algorithm,
            ...
            let size = original.artifact().length() as usize;
            let exact = finite_limit(size);
            let short = finite_limit(size - 1);

            let signed = sign_with_limits(
                bytes(PAYLOAD),
                algorithm,
                bytes(&private),
                None,
            ...
            assert_eq!(rejected.status(), "resource_error");
            assert_eq!(
                rejected.code().as_deref(),
                Some("output_artifact_too_large")
            );
            assert!(!rejected.has_artifact());
            assert!(!rejected.has_modified_payload());

            let decomposed = decompose_with_limits(signed.artifact(), form, outer.clone(), &exact);
            assert_eq!(decomposed.status(), "ok");
            assert_eq!(decomposed.payload().to_vec(), PAYLOAD);
            let rejected = decompose_with_limits(signed.artifact(), form, outer, &short);
            assert_eq!(rejected.status(), "resource_error");
            assert_eq!(rejected.code().as_deref(), Some("input_artifact_too_large"));
            assert!(!rejected.has_payload());
            assert!(!rejected.has_signature_carrier());

            let composed = compose_with_limits(
                decomposed.payload(),
            ...
            let verified =
                verify_with_limits(signed.artifact(), form, algorithm, bytes(&public), &exact);
            assert_eq!(verified.status(), "verified");
            assert_eq!(verified.payload().to_vec(), PAYLOAD);
            let rejected =
                verify_with_limits(signed.artifact(), form, algorithm, bytes(&public), &short);
            assert_eq!(rejected.status(), "resource_error");
            assert_eq!(rejected.code().as_deref(), Some("input_artifact_too_large"));
            assert!(!rejected.has_payload());
            assert!(!rejected.has_algorithm());
        }
    }
}

#[wasm_bindgen_test]
fn bounded_admission_precedes_artifact_processing() {
    init();
    let limits = finite_limit(1);
    let malformed = bytes(&[0xff, 0xff]);
    let rejected = decompose_with_limits(
        malformed.clone(),
        "invalid",
    ...
    assert_eq!(rejected.status(), "resource_error");
    assert_eq!(rejected.code().as_deref(), Some("input_artifact_too_large"));
    let rejected = verify_with_limits(malformed.clone(), "invalid", "invalid", bytes(&[]), &limits);
    assert_eq!(rejected.status(), "resource_error");
    let rejected = compose_with_limits(malformed.clone(), bytes(&[]), "yaml", &limits);
    assert_eq!(rejected.status(), "resource_error");
    let (private, _) = ed25519_keys(7);
    let rejected = sign_with_limits(
        malformed,
        ED25519,
        bytes(&private),
    ...
    // A selected ceiling does not replace the ordinary invocation/state layers.
    let ample = ArtifactResourceLimits::new();
    assert_eq!(
        decompose_with_limits(bytes(PAYLOAD), "invalid", None, &ample).status(),
        "invocation_error"
    );
    assert_eq!(
        verify_with_limits(bytes(PAYLOAD), "yaml", ED25519, bytes(&[]), &ample).status(),
        "invocation_error"
    );
    let (_, public) = ed25519_keys(7);
    assert_eq!(
        decompose_with_limits(bytes(PAYLOAD), "yaml", None, &ample).status(),
        "unsigned"
    );
    assert_eq!(
        verify_with_limits(bytes(PAYLOAD), "yaml", ED25519, bytes(&public), &ample).status(),
        "malformed_attempted_signed"
    );
    assert_eq!(
        verify_with_limits(bytes(&[0xff]), "protobuf", ED25519, bytes(&public), &ample).status(),
        "malformed_attempted_signed"
    );
    let signed = sign(
        bytes(PAYLOAD),
    ...
    let (_, wrong) = ed25519_keys(8);
    assert_eq!(
        verify_with_limits(signed.artifact(), "yaml", ED25519, bytes(&wrong), &ample).status(),
        "signed_but_failed_verification"
    );
}

#[wasm_bindgen_test]
fn bounded_signing_accounts_for_newline_repair_and_keyid_escaping() {
    init();
    let payload = b"name: wasm";
    let (private, public) = ed25519_keys(5);
    let keyid = Some("quoted-\"key".to_owned());
    let original = sign(
        bytes(payload),
        ED25519,
        bytes(&private),
    ...
    assert_eq!(signed.status(), "success");
    assert_eq!(signed.modified_payload().to_vec(), b"name: wasm\n");
    let rejected = sign_with_limits(
        bytes(payload),
        ED25519,
        bytes(&private),
    ...
    assert!(!rejected.has_modified_payload());
    assert_eq!(
        verify_with_limits(signed.artifact(), "yaml", ED25519, bytes(&public), &exact).status(),
        "verified"
    );
}

#[wasm_bindgen_test]
fn optional_policy_does_not_impose_a_protocol_artifact_limit() {
    init();
    let mut payload = vec![b'x'; yaml_sigil_core::DEFAULT_MAX_ARTIFACT_BYTES + 1];
    payload[0] = b'#';
    payload.push(b'\n');
    let (private, public) = ed25519_keys(9);
    let default = ArtifactResourceLimits::new();
    let disabled = default.without_max_artifact_byte_limit();
    assert_eq!(default.max_artifact_bytes(), Some(4_194_304.0));
    assert_eq!(disabled.max_artifact_bytes(), None);

    for (form, outer) in [("yaml", None), ("protobuf", Some("strict".to_owned()))] {
        let signed = sign(bytes(&payload), ED25519, bytes(&private), None, false, form);
        assert_eq!(signed.status(), "success");
        assert_eq!(
            verify(signed.artifact(), form, ED25519, bytes(&public)).status(),
            "verified"
        );
        let split = decompose(signed.artifact(), form, outer.clone());
        assert_eq!(split.status(), "ok");
        assert_eq!(
            compose(split.payload(), split.signature_carrier(), form).status(),
            "success"
        );

        assert_eq!(
            verify_with_limits(signed.artifact(), form, ED25519, bytes(&public), &default).status(),
            "resource_error"
        );
        assert_eq!(
            decompose_with_limits(signed.artifact(), form, outer.clone(), &default).status(),
            "resource_error"
        );
        assert_eq!(
            sign_with_limits(
        ...
        assert_eq!(
            compose_with_limits(split.payload(), split.signature_carrier(), form, &default)
                .status(),
            "resource_error"
        );

        let raised = finite_limit(signed.artifact().length() as usize);
        assert_eq!(
            verify_with_limits(signed.artifact(), form, ED25519, bytes(&public), &raised).status(),
            "verified"
        );
        assert_eq!(
            decompose_with_limits(signed.artifact(), form, outer, &disabled).status(),
            "ok"
        );
        assert_eq!(
            compose_with_limits(split.payload(), split.signature_carrier(), form, &disabled)
                .status(),
            "success"
        );
        assert_eq!(
